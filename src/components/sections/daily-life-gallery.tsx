...
const DailyLifeSlide = ({ segment, index }: { segment: (typeof dailyLifeImages)[number]; index: number }) => {
...
export const DailyLifeGallery = () => {
  return (
    <section
      id="daily-life-gallery"
      className="relative overflow-hidden py-12 sm:py-16 lg:py-20"
      aria-label="Daily life moments at BRUH"
    >
      {/* Background */}
      <div className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute top-1/2 right-[-20%] h-[250px] w-[250px] rounded-full bg-[radial-gradient(circle,_rgba(241,91,34,0.12),_transparent_70%)]" />
      </div>
        ...
        <ScrollReveal direction="up" distance={40}>
          <div className="mx-auto max-w-3xl space-y-6">
            ...
            <p className="text-base leading-relaxed text-[var(--text-muted)] sm:text-lg">
              Between structured activities, children find space for creativity, reflection, and the simple joys of being themselves.
            </p>
          </div>
        </ScrollReveal>
      </div>
      ...
      <div className="mt-16 space-y-0">
        {dailyLifeImages.map((segment, index) => (
          <div key={segment.id}>
            <DailyLifeSlide segment={segment} index={index} />
            {index < dailyLifeImages.length - 1 && (
